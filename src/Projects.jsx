...
export default function Projects() {
    return (
        <div className="mx-auto my-12 min-h-screen max-w-screen-xl px-6 py-12 md:px-12 md:py-16 lg:py-0">
            <div className="group mb-2 inline-flex items-center font-semibold leading-tight text-[#4BD3D0] cursor-pointer">
                ...
                {data.map((item, index) => (
                    <tbody className='border-b border-slate-300 bg-slate-900 backdrop-blur' key={index}>
                        <tr className="border-b border-slate-300 last:border-none text-slate-200">
                            <td className="py-4 pr-4 align-top text-sm"><div className="translate-y-px">{item.year}</div></td>
                            <td className="py-4 pr-4 align-top font-semibold leading-snug"><div>{item.title}</div></td>
                            <td className="hidden py-4 pr-4 align-top text-sm lg:table-cell"><div>{item['made at']}</div></td>
                            <td className="hidden py-4 pr-4 align-top lg:table-cell">
                                <div className='inline-flex flex-wrap space-x-2 space-y-2 text-black font-semibold'>
                                    {item.skills.map ((skill, i) => (
                                        <Skill skill={skill}  key={i} />
                                    ))}
                                </div>
                            </td>
                            <td className="hidden py-4 pr-4 align-top sm:table-cell text-sm group"><a className='group-hover:text-[#4BD3D0]' href={item.link}>{item['link-tag']}</a><i className="fa-solid fa-arrow-up ml-2 text-xs transform rotate-45 group-hover:text-sm group-hover:text-[#4BD3D0]"></i></td>
                        </tr>
                    </tbody>
                ))}
            </table>
